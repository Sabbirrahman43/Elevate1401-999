...
const FocusOverlay = ({ isActive, onExit }: { isActive: boolean; onExit: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 bg-gray-900 z-[60] flex flex-col items-center justify-center text-white p-6 animate-in">
...
const AuthScreen = ({ onLogin }: { onLogin: (name: string) => void }) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900 p-4">
      <Card className="max-w-md w-full py-10 px-8 animate-in">
...
export default function App() {
  ...
  const handleTaskComplete = (task: Task) => {
    if (user?.preferences.notifications) {
        // Optional: Play a sound or small voice prompt
    }
  };
